...
async def main(jsonInput: dict) -> list[dict]:
    ...
    # Summarization Step
    enable_summarization = jsonInput.get('summarization', {}).get('enable_summarization', False)
    if not enable_summarization:
        # Check config.toml as fallback if not in input json
        # Since input_data comes from config in the else block of main, we check if we need to look deeper
        # meaningful structure: input_data['summarization']
        pass
    
    # We expect 'summarization' key in jsonInput if it came from config
    summarization_config = jsonInput.get('summarization', {})
    if summarization_config.get('enable_summarization'):
        api_key = summarization_config.get('anthropic_api_key')
        model = summarization_config.get('model', 'claude-sonnet-4-20250514')
        
        if api_key:
            logger.info(f"🧠 Summarization enabled. Model: {model}")
            summarizer = Summarizer(api_key=api_key, model=model)
            job_attributes = summarizer.summarize_jobs(job_attributes)
        else:
            logger.warning("⚠️ Summarization enabled but no Anthropic API key found in configuration. Skipping.")
